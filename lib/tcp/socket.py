...
def add_send_hdr(msg):
    return b"".join([APICmd.SEND, struct.pack("I", len(msg)), msg])


def add_setpath_hdr(msg):
    return b"".join([APICmd.SET_PATH, struct.pack("I", len(msg)), msg])
...
class SCIONTCPSocket(object):
    ...
    def llsend(self, msg):
        # Low-level send, that does not prepend the sending header (see add_send_hdr()).
        # The caller has to make sure that the header is prepended and all data is sent
        # (_lwip_sock can be non-blocking with these calls).
        with self._lock:
            assert not self._rpc_mode, "Called llsend() in RPC mode"
            try:
                return self._lwip_sock.send(msg)
            except OSError as e:
                raise SCIONTCPError("_lwip_sock.llsend() failed: %s" % e)

    def sendall(self, msg):
        with self._lock:
            assert not self._rpc_mode, "Called sendall() in RPC mode"
            try:
                return self._lwip_sock.sendall(add_send_hdr(msg))
            except OSError as e:
    ...
    def set_path(self, path, first_ip, first_port):
        with self._lock:
            assert not self._rpc_mode, "Called set_path() in RPC mode"
            try:
                msg = b"".join([path.pack(), first_ip.pack(), struct.pack("H", first_port)])
                self._lwip_sock.sendall(add_setpath_hdr(msg))
            except OSError as e:
                raise SCIONTCPError("_lwip_sock.sendall() failed: %s" % e)
